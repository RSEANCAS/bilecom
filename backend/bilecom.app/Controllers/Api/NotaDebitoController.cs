...
namespace bilecom.app.Controllers.Api
{
    ...
    public class NotaDebitoController : ApiController
    {
        ...
        [HttpGet]
        [Route("buscar-notadebito")]
        public DataPaginate<NotaDebitoBe> BuscarNotaDebito(int empresaId, int ambienteSunatId, string nroDocumentoIdentidadCliente, string razonSocialCliente, DateTime fechaEmisionDesde, DateTime fechaEmisionHasta, int draw, int start, int length, string columnaOrden = "NotaDebitoId", string ordenMax = "ASC")
        {
            int totalRegistros = 0;
            var lista = notaDebitoBl.BuscarNotaDebito(empresaId, ambienteSunatId, nroDocumentoIdentidadCliente, razonSocialCliente, fechaEmisionDesde, fechaEmisionHasta, start, length, columnaOrden, ordenMax, out totalRegistros);
            var respuesta = new DataPaginate<NotaDebitoBe>
            {
                data = lista ?? new List<NotaDebitoBe>(),
                draw = draw,
        ...
        public bool GuardarNotaDebito(NotaDebitoBe registro)
        {
            ...
            int? notaDebitoId = null, nroComprobante = null;
            DateTime? fechaHoraEmision = null;
            string totalImporteEnLetras = null;
            bool respuesta = notaDebitoBl.GuardarNotaDebito(registro, out notaDebitoId, out nroComprobante, out fechaHoraEmision, out totalImporteEnLetras);
            ...
            if (respuesta)
            {
                try
                {
                    ...
                    if (notaDebitoId.HasValue) registro.NotaDebitoId = notaDebitoId.Value;
                    ...
                    DebitNoteType debitNoteType = ComprobanteSunat.ObtenerComprobante(registro, ComprobanteSunat.VersionUBL._2_1);

                    string contenidoXml = Generar.GenerarXML(debitNoteType);
                    string hash = null;
                    string contenidoXmlFirmado = Generar.RetornarXmlFirmado("/tns:DebitNote", "urn:oasis:names:specification:ubl:schema:xsd:DebitNote-2", contenidoXml, rutaCertificado, claveCertificado, out hash);
                    registro.Hash = hash;

                    var qr = Generar.GenerarQR(registro.Empresa.Ruc, TipoComprobante.NotaDebito.GetAttributeOfType<DefaultValueAttribute>().Value.ToString(), registro.Serie.Serial, registro.NroComprobante, registro.FechaHoraEmision, registro.Cliente.TipoDocumentoIdentidad.Codigo, registro.Cliente.NroDocumentoIdentidad, registro.TotalIgv, registro.ImporteTotal, registro.Hash);
                    ...
                    string nombreArchivo = $"{registro.Empresa.Ruc}-{TipoComprobante.NotaDebito.GetAttributeOfType<DefaultValueAttribute>().Value}-{registro.Serie.Serial}-{registro.NroComprobante}";
                    ...
                    string rutaCarpetaSunatComprobantes = rutaCarpetaSunatComprobantesBase
                        ...
                        .Replace("{TipoComprobante}", TipoComprobante.NotaDebito.GetAttributeOfType<DefaultValueAttribute>().Value.ToString())
                    ...
                    bool seGuardoRespuestaSunat = notaDebitoBl.GuardarRespuestaSunatNotaDebito(registro);
                }
                catch (Exception ex)
                {
        ...
        [HttpPut]
        [Route("anular-notadebito")]
        public bool AnularNotaDebito(NotaDebitoBe registro)
        {
            bool respuesta = notaDebitoBl.AnularNotaDebito(registro);
            return respuesta;
        }
    }
}
