...
const pageBoleta = {
    ...
    InitEvents: function () {
        $("#txt-nro-documento-identidad-cliente, #txt-razon-social-cliente").keyup(pageBoleta.BtnBuscarClick);
        $("#btn-buscar").click(pageBoleta.BtnBuscarClick);

        let fechaInicial = new Date(fechaActual.getFullYear(), fechaActual.getMonth());
        $("#txt-fecha-emision-desde, #txt-fecha-emision-hasta").datepicker({ format: "dd/mm/yyyy", autoclose: true }).on("changeDate", pageBoleta.BtnBuscarClick);
    ...
    BtnBuscarClick: function (e) {
        ...
        pageBoleta.CreateDataTable("#tbl-lista")
    },
        ...
        bootbox.confirm({
            ...
            callback: function (result) {
                if (result == true) {
                    ...
                    let url = `${urlRoot}api/boleta/anular-boleta`;
                    let data = { EmpresaId: user.Empresa.EmpresaId, BoletaId: id, Usuario: user.Nombre };
                    let headers = { 'Content-Type': 'application/json' };
                    let init = { method: 'PUT', headers, body: JSON.stringify(data) };
                    fetch(url, init)
                        .then(common.ResponseToJson)
                        .then(pageBoleta.ResponseEliminaRegistro);
                }
            }
        })
    },
    ...
    CreateDataTable: function (id) {
        ...
        let ajax = {
            url: `${urlRoot}api/boleta/buscar-boleta`,
            data: {
                empresaId: user.Empresa.EmpresaId,
                ambienteSunatId: ambienteSunatId,
                nroDocumentoIdentidadCliente: pageBoleta.ObtenerNroDocumentoIdentidadCliente,
                razonSocialCliente: pageBoleta.ObtenerRazonSocialCliente,
                fechaEmisionDesde: pageBoleta.ObtenerFechaEmisionDesde,
                fechaEmisionHasta: pageBoleta.ObtenerFechaEmisionHasta
            }
        };
        ...
        let columns = [
            ...
            {
                data: "BoletaId", render: function (data, type, row) {
                    var user = common.ObtenerUsuario();
                    let nombreArchivo = `${user.Empresa.Ruc}-03-${row.Serie.Serial}-${row.NroComprobante}`;
                    let rutaArchivo = `${urlRoot}App_Files/${user.Empresa.Ruc}/Sunat/${ambienteSunatNombre}/Comprobantes/03/${row.Serie.Serial}-${row.NroComprobante}/${nombreArchivo}`;
                    let rutaArchivoCdr = `${urlRoot}App_Files/${user.Empresa.Ruc}/Sunat/${ambienteSunatNombre}/Comprobantes/03/${row.Serie.Serial}-${row.NroComprobante}/R-${nombreArchivo}`;

                    return `${row.FlagAnulado == true ? "" :
                        `<a class="btn btn-sm btn-danger btn-hover-danger fa fa-ban add-tooltip" href="javascript:pageBoleta.BtnAnularClick(${data})" data-original-title="Anular" data-container="body"></a>
