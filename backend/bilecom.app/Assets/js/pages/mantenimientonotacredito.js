...
const columnsDetalle = [
    ...
    {
        data: "NotaCreditoDetalleId", render: function (data, type, row) {
            return `<button type="button" class="btn btn-xs btn-default btn-hover-dark ion-edit add-tooltip btnEditar" onclick='pageMantenimientoNotaCredito.BtnAgregarDetalleClick(event, ${data})' data-original-title="Editar" data-container="body"></button>
                    <button type="button" class="btn btn-xs btn-danger btn-hover-danger ion-trash-a add-tooltip btnEliminar" onclick='pageMantenimientoNotaCredito.BtnEliminarDetalleClick(${data})' data-original-title="Eliminar" data-container="body"></button>`
        }
    },
];
...
const pageMantenimientoNotaCredito = {
        ...
        bootbox.dialog({
            ...
            onShow: function (e) {
                ...
                let ajax = {
                    ...
                    data: {
                        empresaId: empresaId,
                        nroDocumentoIdentidad: pageMantenimientoNotaCredito.ObtenerFiltroClienteNroDocumentoIdentidad,
                        razonSocial: pageMantenimientoNotaCredito.ObtenerFiltroClienteNombres
                    }
                };
                ...
                let columns = [
                    ...
                    { data: "RazonSocial" },
                    {
                        data: "ClienteId", render: function (data, type, row) {
                            return `<button class="btn btn-xs btn-default btn-hover-dark ion-checkmark-circled add-tooltip" onclick='pageMantenimientoNotaCredito.BtnSeleccionarClienteClick(${JSON.stringify(row)})' data-original-title="Seleccionar" data-container="body"></button>`
                        }
                    },
                ];
    ...
    BtnNuevoClienteClick: function () {
        bootbox.dialog({
            title: "Nuevo Cliente",
            message:
                `<form id="frm-notacredito-cliente-nuevo" autocomplete="off">
                    <div class="row">
                        <div class="col-sm-3">
                            <div class="form-group">
                    ...
                    <!--<div class="row">
                        <div class="col-sm-4">
                            <div class="form-group">
                                <button id="btn-cliente-guardar-nuevo" class="btn btn-success" type="button" onclick="pageMantenimientoNotaCredito.BtnGuardarySeleccionar()"><i class="ion-save"></i> Guardar y Seleccionar</button>
                            </div>
                        </div>
                        <div class="col-sm-2">
                            <div class="form-group">
                                <button id="btn-cliente-cancelar-nuevo" class="btn btn-danger" type="button" onClick="pageMantenimientoNotaCredito.BtnCierraNuevoCliente()"><i class="ion-save"></i> Cancelar</button>
                            </div>
                        </div>
                    </div>-->
                </form>`,
            ...
            buttons: {
                guardar: {
                    ...
                    callback: function () {
                        $("#frm-notacredito-cliente-nuevo").trigger("submit");
                        return false;
                    },
    ...
    CmbDepartamentoChange: function () {
        ...
        pageMantenimientoNotaCredito.ResponseProvinciaListar(ProvinciaFiltro, dropdownParent = $("#modal-nuevo-cliente"));

        pageMantenimientoNotaCredito.CmbProvinciaChange();
    },
    ...
    BtnAgregarDetalleClick: function (e, id = null) {
        bootbox.dialog({
            ...
            message:
                `<form id="frm-notacredito-detalle" autocomplete="off">
            ...
            buttons: {
                guardar: {
                    ...
                    callback: function () {
                        $("#frm-notacredito-detalle").trigger("submit");
                        return false;
                    },
            ...
            onShow: function (e) {
                ...
                pageMantenimientoNotaCredito.ResponseTipoProductoListar(tipoProductoLista);
                $("input[name='rbt-detalle-tipo-producto']").change(pageMantenimientoNotaCredito.RbtDetalleTipoProductoChange);

                $("#chk-detalle-avanzado").change(pageMantenimientoNotaCredito.ChkDetalleAvanzado);
                ...
                $("#cmb-detalle-codigo").change(pageMantenimientoNotaCredito.CmbDetalleCodigoChange);
                ...
                $("#cmb-detalle-descripcion").change(pageMantenimientoNotaCredito.CmbDetalleDescripcionChange);

                pageMantenimientoNotaCredito.ResponseUnidadMedidaListar(unidadMedidaLista, dropdownParent = $("#modal-detalle-agregar"));
                //$("input[name='rbt-detalle-tipo-producto']").trigger("change");

                pageMantenimientoNotaCredito.ResponseTipoAfectacionIgvListar(tipoAfectacionIgvLista, dropdownParent = $("#modal-detalle-agregar"));

                let registroExiste = id != null;

                if (registroExiste == true) {
                    let index = detalleLista.findIndex(x => x.NotaCreditoDetalleId == id);
                ...
                pageMantenimientoNotaCredito.ValidarDetalle();
            },
            animateIn: 'zoomInDown',
            animateOut: 'zoomOutUp'
        });
        ...
        bootbox.confirm({
            ...
            callback: function (result) {
                if (result == true) {
                    //debugger;
                    let index = detalleLista.findIndex(x => x.NotaCreditoDetalleId == id);
                    if (detalleLista[index].NotaCreditoDetalleId > 0) detalleListaEliminados.push(detalleLista[index].NotaCreditoDetalleId);
    ...
    CmbDetalleCodigoChange: function () {
        ...
        pageMantenimientoNotaCredito.CargarOtrosCamposProducto(data);
    },
    ...
    ListarDetalle: function () {
        ...
        pageMantenimientoNotaCredito.MostrarTotales();
    },
    ...
    ValidarDetalle: function () {
        $("#frm-notacredito-detalle")
            ...
            .on('success.form.bv', function (e) {
                e.preventDefault();
                pageMantenimientoNotaCredito.GuardarDetalle();
    ...
    ObtenerNotaCreditoDetalleId: function () {
        ...
        let listaIdNegativos = detalleLista.filter(x => x.NotaCreditoDetalleId < 0);
        notaCreditoDetalleId = listaIdNegativos.length == 0 ? 0 : listaIdNegativos.map(x => x.NotaCreditoDetalleId).sort((a, b) => a - b)[0];
    ...
    ObtenerDatos: function () {
        if (notaCreditoId != null) {
            ...
            let url = `${urlRoot}api/notacredito/obtener-notacredito?empresaId=${empresaId}&notaCreditoId=${notaCreditoId}`;
            let init = { method: 'GET' };

            fetch(url, init)
                .then(common.ResponseToJson)
                .then(pageMantenimientoNotaCredito.ResponseObtenerDatos);
        }
    },
    ...
    EnviarFormulario: function () {
        ...
        let totalGravado = detalleLista.filter(x => pageMantenimientoNotaCredito.ObtenerTipoAfectacionIgv(x.TipoAfectacionIgvId).FlagGravado && !pageMantenimientoNotaCredito.ObtenerTipoAfectacionIgv(x.TipoAfectacionIgvId).FlagGratuito).map(x => x.ValorVenta).reduce((a, b) => a + b, 0);
        let totalExonerado = detalleLista.filter(x => pageMantenimientoNotaCredito.ObtenerTipoAfectacionIgv(x.TipoAfectacionIgvId).FlagExonerado && !pageMantenimientoNotaCredito.ObtenerTipoAfectacionIgv(x.TipoAfectacionIgvId).FlagGratuito).map(x => x.ValorVenta).reduce((a, b) => a + b, 0);
        let totalInafecto = detalleLista.filter(x => pageMantenimientoNotaCredito.ObtenerTipoAfectacionIgv(x.TipoAfectacionIgvId).FlagInafecto && !pageMantenimientoNotaCredito.ObtenerTipoAfectacionIgv(x.TipoAfectacionIgvId).FlagGratuito).map(x => x.ValorVenta).reduce((a, b) => a + b, 0);
        let totalExportacion = detalleLista.filter(x => pageMantenimientoNotaCredito.ObtenerTipoAfectacionIgv(x.TipoAfectacionIgvId).FlagExportacion && !pageMantenimientoNotaCredito.ObtenerTipoAfectacionIgv(x.TipoAfectacionIgvId).FlagGratuito).map(x => x.ValorVenta).reduce((a, b) => a + b, 0);
        let totalGratuito = detalleLista.filter(x => pageMantenimientoNotaCredito.ObtenerTipoAfectacionIgv(x.TipoAfectacionIgvId).FlagGratuito).map(x => x.ValorVenta).reduce((a, b) => a + b, 0);
        let totalVentaArrozPilado = detalleLista.filter(x => pageMantenimientoNotaCredito.ObtenerTipoAfectacionIgv(x.TipoAfectacionIgvId).FlagVentaArrozPilado && !pageMantenimientoNotaCredito.ObtenerTipoAfectacionIgv(x.TipoAfectacionIgvId).FlagGratuito).map(x => x.ValorVenta).reduce((a, b) => a + b, 0);
        ...
        let ObjectoJson = {
            ...
            Usuario: user.Nombre,
            ListaNotaCreditoDetalle: detalleLista,
            ListaNotaCreditoDetalleEliminados: detalleListaEliminados
        }

        let url = `${urlRoot}api/notacredito/guardar-notacredito`;
        ...
        fetch(url, init)
            .then(r => r.json())
            .then(pageMantenimientoNotaCredito.ResponseEnviarFormulario);
    },
    ...
    ResponseObtenerDatos: function (data) {
        ...
        pageMantenimientoNotaCredito.LlenarDatosCliente(data.Cliente);
        pageMantenimientoNotaCredito.LlenarDatosPersonal(data.Personal);
        detalleLista = data.ListaNotaCreditoDetalle;
        pageMantenimientoNotaCredito.ListarDetalle();
    },
    ...
    BtnGuardarySeleccionar: function () {
        ...
        let params = JSON.stringify(pageMantenimientoNotaCredito.datosclienteNuevo);
        let headers = { 'Content-Type': 'application/json' };
        let init = { method: 'POST', body: params, headers };
